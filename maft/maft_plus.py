...
from .modeling.meta_arch.open_mamba_head import build_open_mamba
...
class MAFT_Plus(nn.Module):
    ...
    def _freeze(self, ):
        for name, param in self.named_parameters():
            if "open_mamba" in name and "sem_seg_head" not in name:
                param.requires_grad = True
            if "open_mamba" not in name or "sem_seg_head" in name:
    ...
    def from_config(cls, cfg):
        ...
        open_mamba = build_open_mamba(cfg, cfg.MODEL.OPEN_MAMBA.NAME)

        # loss weights
        class_weight = cfg.MODEL.MASK_FORMER.CLASS_WEIGHT
        info_nce_weight = cfg.MODEL.OPEN_MAMBA.INFO_NCE_WEIGHT

        weight_dict = {"loss_ce": class_weight, "loss_info_nce": info_nce_weight}
        ...
        return {
            ...
            "iou_threshold": cfg.MODEL.OPEN_MAMBA.IOU_THRESHOLD,
            "mask_threshold": cfg.MODEL.OPEN_MAMBA.MASK_THRESHOLD,
            "num_gt_masks": cfg.MODEL.OPEN_MAMBA.NUM_GT_MASKS,
            "num_pred_masks": cfg.MODEL.OPEN_MAMBA.NUM_PRED_MASKS,
        }
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                ...
                targets, masks, labels = self.prepare_targets_for_openmamba(gt_instances, images)
            else:
                targets = None
            ...
            src_masks, target_masks, mask_labels = self.match_via_atss(mask_pred_results, mask_cls_results, targets, topk=9, max_matches=self.num_pred_masks)
            ...
            outputs = self.open_mamba(clip_vis_dense, mask_pred, text_features, self.training)
            ...
            loss_info_nce = self.info_nce_loss(pooled_clip_feature[:, 16:24, :], pooled_clip_feature[:, 24:, :])
            ...
            outputs = self.open_mamba(clip_vis_dense, binary_masks, text_features, self.training)
    ...
    def prepare_targets_for_openmamba(self, targets, images):
        h_pad, w_pad = images.tensor.shape[-2:]
        new_targets = []
        masks_list = []
        labels_list = []
    ...
    def match_via_atss(self,
                      ...
                      max_matches=8):
        """
        ATSS-like dynamic IoU matcher for mask queries.
        For each GT mask, collects topk IoUs, computes mean+std threshold,
        selects preds with IoU >= that threshold as positives.
        Returns exactly max_matches pairs per image (padded if needed).
        """
        B, Nq, Hq, Wq = mask_pred_results.shape
        matched_src_masks = []
        ...
        for b in range(B):
            ...
            tgt_resized = F.interpolate(
                tgt_mask[:, None].float(), size=(Hq, Wq),
                mode='bilinear', align_corners=False
            ).squeeze(1)                                                   
            tgt_flat = tgt_resized.flatten(1)                                

            with torch.no_grad():
                ious = compute_mask_iou(pred_mask_flat, tgt_flat)               
            ...
            for j in range(Ki):
                iou_j = ious[:, j]                                         
                num = min(topk, iou_j.numel())
                topk_vals, _ = torch.topk(iou_j, num)
                thr = topk_vals.mean() + topk_vals.std()
                valid = (iou_j >= thr).nonzero(as_tuple=True)[0]
                for p in valid.tolist():
                    sel_pred_idx.append(p)
                    sel_tgt_idx.append(j)

            if len(sel_pred_idx) > max_matches:
                perm = torch.randperm(len(sel_pred_idx))[:max_matches]
                sel_pred_idx = [sel_pred_idx[i] for i in perm]
                sel_tgt_idx  = [sel_tgt_idx[i]  for i in perm]

            L = len(sel_pred_idx)
            if L < max_matches:
                pad_n = max_matches - L
                src = torch.cat([
                    pred_mask[sel_pred_idx],
                    torch.zeros((pad_n, Hq, Wq), device=pred_mask.device)
                ], dim=0)
                tgt_sel = tgt_resized[sel_tgt_idx] if L>0 else torch.empty((0, Hq, Wq), device=tgt_resized.device)
                tgt_pad = torch.zeros((pad_n, Hq, Wq), device=tgt_resized.device)
                tgt = torch.cat([tgt_sel, tgt_pad], dim=0)
                lbl_sel = tgt_label[sel_tgt_idx] if L>0 else torch.empty((0,), dtype=tgt_label.dtype, device=tgt_label.device)
                lbl_pad = torch.full((pad_n,), -1, dtype=tgt_label.dtype, device=tgt_label.device)
                lbl = torch.cat([lbl_sel, lbl_pad], dim=0)
            else:
                src = pred_mask[sel_pred_idx[:max_matches]]           
                tgt = tgt_resized[sel_tgt_idx[:max_matches]]          
                lbl = tgt_label[sel_tgt_idx[:max_matches]]           

            matched_src_masks.append(src)
            matched_target_masks.append(tgt)
    ...
    def cross_entropy_loss(self, mask_cls_results, labels):
        if torch.all(labels == -1):
            ...
            logits = mask_cls_results.transpose(1, 2)
            loss_ce = F.cross_entropy(logits, labels.to(torch.int64), ignore_index=-1)

        return {"loss_ce": loss_ce}

    def info_nce_loss(self, pred_feats, gt_feats, temperature=0.03):
        B, K, D = pred_feats.shape
        N = B * K

        pred = F.normalize(pred_feats.reshape(N, D), dim=-1) 
        gt = F.normalize(gt_feats.reshape(N, D), dim=-1) 
        feats = torch.cat([pred, gt], dim=0)                  
        logits = torch.matmul(feats, feats.t()) / temperature  
        mask = torch.eye(2*N, device=logits.device).bool()
        logits = logits.masked_fill(mask, float('-inf'))
        labels = (torch.arange(2*N, device=logits.device) + N) % (2*N)
        loss = F.cross_entropy(logits, labels)

        return {"loss_info_nce": loss}
