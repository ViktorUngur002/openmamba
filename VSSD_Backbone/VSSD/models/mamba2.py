...
import copy
try:
    from mamba_util import PatchMerging,SimplePatchMerging, Stem, SimpleStem, Mlp, SwiGLU_Mlp
except:
    from .mamba_util import PatchMerging, SimplePatchMerging, Stem, SimpleStem, Mlp, SwiGLU_Mlp
...
class CrossAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., textual_dim=640):
        super().__init__()
        inner_dim = dim_head * heads
        self.heads = heads
        self.scale = dim_head ** -0.5
        
        # Projection for visual queries.
        self.to_q = nn.Linear(dim, inner_dim, bias=False)
        
        self.to_kv = nn.Linear(textual_dim, inner_dim * 2, bias=False)
        
        # Output linear projection.
        self.to_out = nn.Linear(inner_dim, dim)
        self.dropout = nn.Dropout(dropout)
        self.guidance_scale = 1.0  # optional extra scaling if needed
    ...
    def forward(self, x, text, H, W):
        ...
        q = rearrange(q, "b n (h d) -> b h n d", h=self.heads)  # (B, heads, N, d)
        k = rearrange(k, "b t (h d) -> b h t d", h=self.heads)  # (B, heads, T, d)
        v = rearrange(v, "b t (h d) -> b h t d", h=self.heads)  # (B, heads, T, d)
        
        dots = torch.einsum("bhid,bhjd->bhij", q, k) * self.scale  # (B, heads, N, T)
        attn = dots.softmax(dim=-1)
        attn = self.dropout(attn)
        out = torch.einsum("bhij,bhjd->bhid", attn, v)  # (B, heads, N, d)
...
class Mamba2_txt(nn.Module):
    def __init__(
        ...
        if ngroups == -1:
            self.ngroups = self.d_inner // self.headdim
        ...
        self.ssd_positve_dA = True

        d_in_proj_vis = 2 * self.d_inner + self.nheads  
        self.in_proj_visual = nn.Linear(self.d_model, d_in_proj_vis, bias=bias, **factory_kwargs)

        if text_proj_dim is None:
            text_proj_dim = 2 * self.ngroups * self.d_state  
        self.in_proj_text = nn.Linear(textual_dim, text_proj_dim, bias=bias, **factory_kwargs)
    ...
    def non_causal_linear_attn_cross(self, Q, dt, A, K, V, D):
        """
        Modified non-causal attention for cross-modal use:
        Q: (B, Lq, H, D)  from visual (queries)
        dt: (B, Lq, nheads) from visual
        A: (nheads,) scalar factors
        K: (B, T, d_state) from text (keys)
        V: (B, T, d_state) from text (values)
        D: (nheads,) skip parameter
        
        Assumes d_state == D (i.e. headdim) for simplicity.
        """
        batch, Lq, head, D_ = Q.shape 
        Q = Q.permute(0, 2, 1, 3) 
        dt = dt.permute(0, 2, 1)
        H = self.nheads
        K = K.unsqueeze(1).expand(batch, H, -1, -1)  
        V = V.unsqueeze(1).expand(batch, H, -1, -1) 
        
        dA = dt.unsqueeze(-1) * A.view(1, -1, 1, 1) 
        ...
        skip = Q 
        out = out + skip * D.view(1, -1, 1, 1) 
        out = out.permute(0, 2, 1, 3).contiguous()
        return out

    def forward(self, visual, text, H, W):
        """
        visual: (B, Lq, C)   where Lq = H * W (flattened image features)
        text: (B, C, T)      textual features from CLIP (T tokens); will be transposed to (B, T, C)
        H, W: spatial dimensions for the visual feature map.
        Returns: (B, Lq, C) the enhanced visual features.
        """
        batch, Lq, _ = visual.shape

        # Project visual features
        vis_proj = self.in_proj_visual(visual)

        z, x_v, dt = torch.split(vis_proj, [self.d_inner, self.d_inner, self.nheads], dim=-1)
        dt = F.softplus(dt + self.dt_bias)
        
        x_v_2d = x_v.view(batch, H, W, self.d_inner).permute(0, 3, 1, 2).contiguous()
        x_v_2d = self.act(self.conv2d(x_v_2d))
        x_v = x_v_2d.permute(0, 2, 3, 1).reshape(batch, Lq, self.d_inner).contiguous()

        Q = rearrange(x_v, "b l (h d) -> b l h d", h=self.nheads)
        
        # Project text features
        text_proj = self.in_proj_text(text)
        K_text, V_text = torch.split(text_proj, self.ngroups * self.d_state, dim=-1)
        K = K_text  
        V = V_text 
        
        A = -torch.exp(self.A_log)  
        y = self.non_causal_linear_attn_cross(Q, dt, A, K, V, self.D) 
        y = rearrange(y, "b l h d -> b l (h d)")
        
        y = self.norm(y)

        # Multiply by gate branch
        y = y * z  
        out = self.out_proj(y)
        return out
...
class MCABlock(nn.Module):
    ...
    Args:
        ...
        attn_type (str): Type of attention to use, either 'standard' or 'mamba2'.
    ...
    def __init__(self, dim, textual_dim, input_resolution, attn_type, num_heads, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
                 act_layer=nn.SiLU, ssd_expansion=2, ssd_ngroups=1, ssd_chunk_size=256,
                 linear_attn_duality=False, d_state = 64):
        super().__init__()
        self.dim = dim
        ...
        self.norm_txt = nn.LayerNorm(textual_dim)
        if attn_type == 'standard':
            self.attn = CrossAttention(dim=dim, heads=num_heads, dim_head=dim // num_heads, dropout=drop, textual_dim=textual_dim)
        elif attn_type == 'mamba2':
            self.attn = Mamba2_txt(d_model=dim, textual_dim=textual_dim, expand=ssd_expansion, headdim= dim*ssd_expansion // num_heads,
                                ngroups=ssd_ngroups, chunk_size=ssd_chunk_size,
                                linear_attn_duality=linear_attn_duality, d_state=d_state)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.norm2 = nn.LayerNorm(dim)
        self.mlp = SwiGLU_Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)

    def forward(self, x, text, H=None, W=None):
        B, L, C = x.shape
        if H & W is None:
        ...
        # SSD or Cross Attention
        x = self.attn(x, text, H, W)
...
class MambaCrossAttention(nn.Module):
    def __init__(self, dims, textual_dim, input_resolutions, attn_types, num_heads, mlp_ratio, 
                 qkv_bias, drop, drop_path, use_checkpoint, ssd_expansion, ssd_ngroups, ssd_chunk_size, 
                 linear_attn_duality, d_states):
        super().__init__()
        
        assert len(dims) == len(input_resolutions) == len(d_states), "dims and input_resolutions and d_states must have the same length."
        self.num_stages = len(dims)
        self.depth = len(attn_types)
        self.use_checkpoint = use_checkpoint
        
        self.blocks_per_stage = nn.ModuleList()
        for stage in range(self.num_stages):
            stage_blocks = nn.ModuleList()
            for d in range(len(attn_types)):
                block = MCABlock(
                    dim=dims[stage],
                    textual_dim=textual_dim,
                    input_resolution=input_resolutions[stage],
                    attn_type=attn_types[d],
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    drop=drop,
                    ...
                    d_state=d_states[stage]
                )
                stage_blocks.append(block)
            self.blocks_per_stage.append(stage_blocks)

    @classmethod
    def from_config(cls, cfg):
        return {
            "dims": cfg.MODEL.OPEN_MAMBA.MCA.DIMS,
            "textual_dim": cfg.MODEL.OPEN_MAMBA.MCA.TEXTUAL_DIM,
            "input_resolutions": cfg.MODEL.OPEN_MAMBA.MCA.INPUT_RESOLUTIONS,
            "attn_types": cfg.MODEL.OPEN_MAMBA.MCA.ATTN_TYPES,
            "num_heads": cfg.MODEL.OPEN_MAMBA.MCA.NUM_HEADS,
            "mlp_ratio": cfg.MODEL.OPEN_MAMBA.MCA.MLP_RATIO,
            "qkv_bias": cfg.MODEL.OPEN_MAMBA.MCA.QKV_BIAS,
            "drop": cfg.MODEL.OPEN_MAMBA.MCA.DROP,
            "drop_path": cfg.MODEL.OPEN_MAMBA.MCA.DROP_PATH,
            "ssd_expansion": cfg.MODEL.OPEN_MAMBA.MCA.SSD_EXPANSION,
            "ssd_ngroups": cfg.MODEL.OPEN_MAMBA.MCA.SSD_NGROUPS,
            "ssd_chunk_size": cfg.MODEL.OPEN_MAMBA.MCA.SSD_CHUNK_SIZE,
            "linear_attn_duality": cfg.MODEL.OPEN_MAMBA.MCA.LINEAR_ATTN_DUALITY,
            "d_states": cfg.MODEL.OPEN_MAMBA.MCA.D_STATES,
            "use_checkpoint": cfg.MODEL.OPEN_MAMBA.MCA.USE_CHECKPOINT
        }
    
    def forward(self, features, text):
        outs = []
        for stage_idx, stage_blocks in enumerate(self.blocks_per_stage):
            feat = features[stage_idx]
            B, C, H, W = feat.shape
            feat = rearrange(feat, "b c h w -> b (h w) c") 
            for block in stage_blocks:
                feat = block(feat, text, H, W)
            feat_out = rearrange(feat, "b (h w) c -> b c h w", h=H, w=W).contiguous()
...
class Backbone_VMAMBA2(VMAMBA2):
    def __init__(self, image_size, patch_size, in_chans, embed_dim,
                 depths, num_heads, mlp_ratio,
                 drop_rate, drop_path_rate,
                 simple_downsample, simple_patch_embed,
                 ssd_expansion, ssd_ngroups, ssd_chunk_size,
                 linear_attn_duality, lepe, attn_types,
                 bidirection, d_state, ssd_positive_dA,
                 out_indices=(0,1,2,3), pretrained=None, **kwargs):
        super().__init__(
            ...
            simple_downsample=simple_downsample,
            simple_patch_embed=simple_patch_embed,
            ssd_expansion=ssd_expansion, ssd_ngroups=ssd_ngroups,
            ssd_chunk_size=ssd_chunk_size,
            linear_attn_duality=linear_attn_duality,
            lepe=lepe, attn_types=attn_types,
            bidirection=bidirection, d_state=d_state,
    ...
    def from_config(cls, cfg):
        return {
            "image_size":          cfg.MODEL.OPEN_MAMBA.VSSD.IMAGE_SIZE,
            "patch_size":          cfg.MODEL.OPEN_MAMBA.VSSD.PATCH_SIZE,
            "in_chans":            cfg.MODEL.OPEN_MAMBA.VSSD.IN_CHANS,
            "embed_dim":           cfg.MODEL.OPEN_MAMBA.VSSD.EMBED_DIM,
            "depths":              cfg.MODEL.OPEN_MAMBA.VSSD.DEPTHS,
            "num_heads":           cfg.MODEL.OPEN_MAMBA.VSSD.NUM_HEADS,
            "mlp_ratio":           cfg.MODEL.OPEN_MAMBA.VSSD.MLP_RATIO,
            "drop_rate":           cfg.MODEL.OPEN_MAMBA.VSSD.DROP_RATE,
            "drop_path_rate":      cfg.MODEL.OPEN_MAMBA.VSSD.DROP_PATH_RATE,
            "simple_downsample":   cfg.MODEL.OPEN_MAMBA.VSSD.SIMPLE_DOWNSAMPLE,
            "simple_patch_embed":  cfg.MODEL.OPEN_MAMBA.VSSD.SIMPLE_PATCH_EMBED,
            "ssd_expansion":       cfg.MODEL.OPEN_MAMBA.VSSD.SSD_EXPANSION,
            "ssd_ngroups":         cfg.MODEL.OPEN_MAMBA.VSSD.SSD_NGROUPS,
            "ssd_chunk_size":      cfg.MODEL.OPEN_MAMBA.VSSD.SSD_CHUNK_SIZE,
            "linear_attn_duality": cfg.MODEL.OPEN_MAMBA.VSSD.LINEAR_ATTN_DUALITY,
            "lepe":                cfg.MODEL.OPEN_MAMBA.VSSD.LEPE,
            "attn_types":          cfg.MODEL.OPEN_MAMBA.VSSD.ATTN_TYPES,
            "bidirection":         cfg.MODEL.OPEN_MAMBA.VSSD.BIDIRECTION,
            "d_state":             cfg.MODEL.OPEN_MAMBA.VSSD.D_STATE,
            "ssd_positive_dA":     cfg.MODEL.OPEN_MAMBA.VSSD.SSD_POSITIVE_DA,
        }
