...
from maft import (
    ...
    add_fcclip_config,
    add_open_mamba_config,
)

from detectron2.evaluation import SemSegEvaluator, COCOPanopticEvaluator, DatasetEvaluators
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        mapper = cfg.INPUT.DATASET_MAPPER_NAME

        # Semantic segmentation evaluator
        if mapper in ["mask_former_semantic", "mask_former_panoptic"] \
        ...
        # Panoptic segmentation evaluator for ADE20K
        if mapper == "mask_former_panoptic" \
           and evaluator_type == "ade20k_panoptic_seg" \
           and cfg.MODEL.MASK_FORMER.TEST.PANOPTIC_ON:
            evaluator_list.append(
                COCOPanopticEvaluator(
                    dataset_name,
                    output_dir=output_folder,    # <-- use output_dir, not output_folder
                )
            )

        if not evaluator_list:
            raise NotImplementedError(
                f"No evaluator for mapper={mapper} on dataset={dataset_name}"
            )
